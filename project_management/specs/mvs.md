...
- M-9.28: The Try-it-out table shall mark clips tagged `in_operating_envelope` as Calibrated and shall show live Field Accuracy, False Positives (Unrelated-labeled count over the Field pool), Errors (error-tagged count over listed real-world clips), Calibrated Accuracy with correct/total clip counts, and Total Trip Time in hours. Field Accuracy shall be the auto-vs-hand match rate on labeled complete, rolling, run-through, and Unrelated clips, excluding `synthetic` and `error`. Unrelated (false positive) hand labels shall count in that Field fraction (a miss unless the model also predicted Unrelated). Clips tagged `error` shall appear with Label Error and shall not count toward Field, Calibrated, or False Positive lines. Total Trip Time shall be the sum of confirmed S3 `trip_segment` durations, cached locally, and refreshed when the public clip list fetch succeeds and the cached snapshot changed.
...
- M-9.53: The frontend shall present live Field Accuracy (excluding synthetic and error clips, including Unrelated false positives in the labeled set) and Calibrated Accuracy with per-class percentages and clip counts.
