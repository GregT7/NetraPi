...
def test_media_object_key_clip_uses_session_month() -> None:
    ...
    assert (
        ...
        == "Aug-2026/driving_session_id_1/clips/clip-10/clip.mp4"
    )
...
def test_directory_clip_key_and_sidecars() -> None:
    video = "Aug-2026/driving_session_id_1/clips/clip-10/clip.mp4"
    legacy = "Aug-2026/driving_session_id_1/clips/clip-10.mp4"
    assert is_directory_clip_key(video) is True
    assert is_directory_clip_key(legacy) is False
    assert clip_sidecar_key(video, "areas.json") == (
        "Aug-2026/driving_session_id_1/clips/clip-10/areas.json"
    )
    assert clip_sidecar_keys(legacy) is None
    assert clip_sidecar_keys(video) == (
        "Aug-2026/driving_session_id_1/clips/clip-10/areas.json",
        "Aug-2026/driving_session_id_1/clips/clip-10/motion.json",
        "Aug-2026/driving_session_id_1/clips/clip-10/transitions.json",
    )
