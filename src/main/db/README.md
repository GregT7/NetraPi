...
Revision **0002** seeds without explicit primary keys (FKs via lookup after insert) so PostgreSQL SERIAL sequences stay correct. After rewriting early revisions, wipe Pi SQLite / Supabase and `upgrade head` from scratch — do not upgrade an old DB onto a rewritten `0002`.
